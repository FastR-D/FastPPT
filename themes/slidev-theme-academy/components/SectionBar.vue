...
const barMode = computed(() => {
  ...
  return getSectionBarMode(slide, $slidev.configs.sectionBarMode ?? 'full')
})
...
function setItemRef(el: unknown, i: number) {
  itemEls.value[i] = el instanceof HTMLElement ? el : null
}
...
function onProgressClick(e: MouseEvent, i: number) {
  ...
  const track = wrapper.querySelector<HTMLElement>('.ustc-progress-track')
  ...
  if (target) $slidev.nav.go(target.no)
}
</script>
