...
3. **Measure** with FastPPT MCP `get_preview_status` and `inspect_overflow` (§7).
...
5. **Look** at borderline slides in the FastPPT live preview for the things numbers
   cannot catch (balance, taste, "feels off", and **legibility**: body text and figure
   labels readable at audience scale — overflow evidence does not check font size or
   contrast).
...
FastPPT MCP `inspect_overflow` is the render-truth measurement tool. It asks the active
SPA to inspect the slide inside its hidden Slidev `/print` iframe. Gateway, MCP, and this
theme Skill never launch Chromium. **It does not judge taste — this file does.**

Call `get_preview_status` first, then call `inspect_overflow` with `path` and a one-based
`slide` number for every changed or suspicious slide. An active FastPPT SPA is required;
if it is absent, inspection reports unavailable instead of falling back to a server-side
browser.
...
**Workflow:** inspect after drafting → resolve every overflow finding → re-inspect →
review borderline slides in the live preview, checking the things the measurement cannot
judge (legibility, contrast, balance). With no active SPA, fall back to the rules above
as heuristics and report that render-based inspection remains unavailable — but they're
