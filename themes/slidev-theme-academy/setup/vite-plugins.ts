...
interface ThemeModuleNode {
  id?: string | null
  url?: string
}

interface ThemeModuleGraph {
  urlToModuleMap?: Map<string, ThemeModuleNode>
  idToModuleMap?: Map<string, ThemeModuleNode>
  invalidateModule?(module: ThemeModuleNode): void
}

interface ThemeHotUpdateContext extends NotesOnlyHotUpdateContext {
  modules?: ThemeModuleNode[]
  server?: { moduleGraph?: ThemeModuleGraph }
}

interface ThemeHotUpdateHandlerThis {
  environment?: { moduleGraph?: ThemeModuleGraph }
}
...
export function resolveNotesOnlyHmrFiles(config: AcademyThemeViteConfig = {}): string[] {
  const configured = Array.isArray(config.academyTheme?.notesOnlyHmrFiles)
    ? config.academyTheme.notesOnlyHmrFiles
    : notesOnlyHmrFiles
  const extra = Array.isArray(config.academyTheme?.extraNotesOnlyHmrFiles)
    ? config.academyTheme.extraNotesOnlyHmrFiles
    : []
...
export function collectSlideHmrModules(moduleGraph?: ThemeModuleGraph): ThemeModuleNode[] {
  const result: ThemeModuleNode[] = []
  const seen = new Set<ThemeModuleNode>()

  const visit = (mod: ThemeModuleNode | undefined) => {
...
export function handleSlideMarkdownHotUpdate(
  ctx: ThemeHotUpdateContext,
  moduleGraph = ctx.server?.moduleGraph,
) {
...
export default () => {
  ...
  return {
    ...
    configResolved(config: AcademyThemeViteConfig) {
      resolvedNotesOnlyHmrHandler = createNotesOnlyHmrHandler({
        ...(config.root === undefined ? {} : { projectRoot: config.root }),
        patterns: resolveNotesOnlyHmrFiles(config),
      })
    },
    hotUpdate: {
      order: 'post',
      async handler(this: ThemeHotUpdateHandlerThis, ctx: ThemeHotUpdateContext) {
    ...
    async handleHotUpdate(ctx: ThemeHotUpdateContext) {
