...
interface ThemeSlideSource {
  content: string
  frontmatter?: Record<string, unknown>
}

interface ThemeMarkdownSlide {
  source: ThemeSlideSource
}

interface ThemeMarkdownTransformContext {
  options: {
    data: {
...
export function extractTypstFences(src: string): TypstFence[] {
  ...
  while (i < src.length) {
    ...
    while (scan < src.length) {
      ...
      if (closeFence && closeFence[0] === fenceChar && closeFence.length >= fenceLen) {
...
function typstTransformer(ctx: ThemeMarkdownTransformContext) {
  const snippets = extractTypstFences(ctx.s.original)
  const typst = (ctx.options.data.headmatter.typst ??= {})
  const svgs = snippets.map(({ code }) => renderTypst(code, typst))
  for (let i = snippets.length - 1; i >= 0; i--) {
    const snippet = snippets[i]!
    ctx.s.overwrite(snippet.start, snippet.end, svgs[i] ?? '')
  }
}
...
function isBackupSlide(slide: ThemeMarkdownSlide | undefined): boolean {
  return slide?.source.frontmatter?.layout === 'backup'
}

function numberingTransformer(ctx: ThemeMarkdownTransformContext) {
  ...
  for (let i = 0; i < myIndex; i++) {
    ...
    const opts = inAppendix
      ...
      : undefined
    const priorSlide = slides[i]
    if (priorSlide) {
      counters = injectBlockNumbers(priorSlide.source.content, counters, opts).counters
    }
  }
