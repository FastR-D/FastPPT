import { getLayout, getSectionTitle, type SlideLike } from './layoutHelper.ts'
...
export function isSectionBodySlide(slide: SlideLike): boolean {
...
export function buildSectionGroups(
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    const layout = getLayout(slide)
    const no =
      slide && 'no' in slide && typeof slide.no === 'number' ? slide.no : i + 1
