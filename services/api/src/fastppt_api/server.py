...
from fastppt_runtime.design_packs import normalize_directory_upload_path
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def _headers(self, status: int, content_type: str, content_length: int, *, cookie: str | None = None, content_disposition: str | None = None) -> None:
    ...
    def _conflict_code(message: str) -> str:
        ...
        for code in ("design_confirmation_required", "template_capacity_exceeded", "package_hash_mismatch", "protected_asset_conflict", "artifact_missing", "artifact_commit_incomplete"):
            if code in lowered:
                return code.upper()
    ...
    def do_GET(self) -> None:
        ...
        if match:
            ...
            return
        match = self._route(rf"/api/v1/projects/{PROJECT}/v2/task1/recovery/(?P<idempotency_key>[^/]+)", path)
        if match:
            if not self._require_project(owner_id, match["project_id"]):
                return
            self._json(HTTPStatus.OK, self.runtime.service.task1_recover(owner_id, match["project_id"], match["idempotency_key"]))
            return
        match = self._route(rf"/api/v1/projects/{PROJECT}/audit-export", path)
        if match:
            if not self._require_project(owner_id, match["project_id"]):
                return
            content, file_name = self.runtime.service.export_audit_package(owner_id, match["project_id"])
            self._headers(HTTPStatus.OK, "application/zip", len(content), content_disposition=f'attachment; filename="{file_name}"')
            self.wfile.write(content)
            return
        ...
        if match:
            ...
            return
        match = self._route(rf"/api/v1/projects/{PROJECT}/agent-runs/(?P<agent_run_id>[^/]+)/context", path)
        if match:
            if not self._require_project(owner_id, match["project_id"]):
                return
            inspection = self.runtime.service.inspect_agent_run(owner_id, match["project_id"], match["agent_run_id"])
            self._json(HTTPStatus.OK, {key: inspection.get(key) for key in ("status", "agent_run", "context_manifest", "truncation_report", "output_artifact_ids")})
            return
        match = self._route(rf"/api/v1/projects/{PROJECT}/agent-runs/(?P<agent_run_id>[^/]+)/prompt", path)
        if match:
            if not self._require_project(owner_id, match["project_id"]):
                return
            inspection = self.runtime.service.inspect_agent_run(owner_id, match["project_id"], match["agent_run_id"])
            self._json(HTTPStatus.OK, {key: inspection.get(key) for key in ("status", "agent_run", "prompt_envelope", "final_prompt")})
            return
        ...
        if match:
            ...
            return
        match = self._route(rf"/api/v1/projects/{PROJECT}/design-packs", path)
        if match:
            if not self._require_project(owner_id, match["project_id"]):
                return
            include_all = query.get("all_private") == ["1"]
            if include_all and not self._require_admin(user):
                return
            kind = query.get("kind", [None])[0]
            self._json(HTTPStatus.OK, {"design_packs": self.runtime.service.list_design_packs(owner_id, match["project_id"], pack_kind=kind, include_all_private=include_all)})
            return
        match = self._route(rf"/api/v1/projects/{PROJECT}/design-packs/(?P<pack_id>[^/]+)", path)
        if match:
            if not self._require_project(owner_id, match["project_id"]):
                return
            include_all = query.get("all_private") == ["1"]
            if include_all and not self._require_admin(user):
                return
            self._json(HTTPStatus.OK, self.runtime.service.get_design_pack(owner_id, match["project_id"], match["pack_id"], include_all_private=include_all))
            return
        match = self._route(rf"/api/v1/projects/{PROJECT}/design-packs/(?P<pack_id>[^/]+)/previews/(?P<preview_index>\d+)", path)
        if match:
            if not self._require_project(owner_id, match["project_id"]):
                return
            content, media_type = self.runtime.service.design_pack_preview(owner_id, match["project_id"], match["pack_id"], int(match["preview_index"]))
            self._headers(HTTPStatus.OK, media_type, len(content))
            self.wfile.write(content)
            return
        ...
        if match:
            ...
            return
        match = self._route(rf"/api/v1/projects/{PROJECT}/sessions/(?P<session_id>session_[a-f0-9]{{32}})/design-selection", path)
        if match:
            if not self._require_project(owner_id, match["project_id"]):
                return
            self._json(HTTPStatus.OK, self.runtime.service.get_session_design_selection(owner_id, match["project_id"], match["session_id"]))
            return
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/api/v1/projects":
                ...
                return
            match = self._route(rf"/api/v1/projects/{PROJECT}/v2/task1/(?P<action>preview|generate|recover)", path)
            if match:
                key = self._require_idempotency_key()
                if not key:
                    return
                body["idempotency_key"] = key
                if match["action"] == "preview":
                    self._json(HTTPStatus.OK, self.runtime.service.task1_preview(owner_id, match["project_id"], body))
                elif match["action"] == "generate":
                    self._json(HTTPStatus.CREATED, self.runtime.service.task1_generate(owner_id, match["project_id"], body))
                else:
                    self._json(
                        HTTPStatus.OK,
                        self.runtime.service.task1_recover(owner_id, match["project_id"], key),
                    )
                return
            match = self._route(rf"/api/v1/projects/{PROJECT}/agent-runs", path)
            if match:
                ...
                return
            match = self._route(rf"/api/v1/projects/{PROJECT}/agent-runs/(?P<agent_run_id>[^/]+)/replay", path)
            if match:
                if not self._require_project(owner_id, match["project_id"]):
                    return
                execute = bool(body.get("execute", False))
                if execute and not self._require_idempotency_key():
                    return
                self._json(HTTPStatus.OK if not execute else HTTPStatus.ACCEPTED, self.runtime.service.replay_agent_run(owner_id, match["project_id"], match["agent_run_id"], execute=execute))
                return
            ...
            if match:
                if not self._require_idempotency_key():
                    return
                self._json(HTTPStatus.CREATED, self.runtime.service.select_design_pack(owner_id, match["project_id"], match["session_id"], str(body.get("pack_id")) if body.get("pack_id") else None, str(body.get("pack_kind") or "")))
                return
            match = self._route(rf"/api/v1/projects/{PROJECT}/design-packs/import", path)
            if match:
                if not self._require_idempotency_key():
                    return
                if body.get("directory_files") is not None:
                    entries = body.get("directory_files")
                    if not isinstance(entries, list):
                        raise ValueError("directory_files must be an array")
                    ...
                    for entry in entries:
                        if not isinstance(entry, dict) or not isinstance(entry.get("path"), str) or not entry.get("path"):
                            raise ValueError("directory_files entries require a relative path")
                        try:
                            path = normalize_directory_upload_path(entry["path"])
                        except ValueError as exc:
                            raise ValueError("directory_files path is invalid") from exc
                        try:
                            value = base64.b64decode(str(entry.get("content_base64", "")), validate=True)
                        except ValueError as exc:
                            raise ValueError("directory_files content_base64 is invalid") from exc
                        path_key = path.casefold()
                        if path_key in normalized_paths:
                            raise ValueError("directory_files contains duplicate or case-colliding paths")
                        normalized_paths.add(path_key)
                        decoded_entries.append((path, value))
                    first_segment = decoded_entries[0][0].split("/", 1)[0] if decoded_entries else ""
                    directory_prefix = f"{first_segment}/" if first_segment and all(path.startswith(f"{first_segment}/") for path, _value in decoded_entries) else ""
                    content: dict[str, bytes] = {}
                    imported_paths: set[str] = set()
                    for path, value in decoded_entries:
                        imported_path = path.removeprefix(directory_prefix)
                        path_key = imported_path.casefold()
                        if not imported_path or path_key in imported_paths:
                            raise ValueError("directory_files contains duplicate or case-colliding paths")
                        imported_paths.add(path_key)
                        content[imported_path] = value
                else:
                    try:
                        content = base64.b64decode(str(body.get("content_base64", "")), validate=True)
                    except ValueError as exc:
                        raise ValueError("content_base64 is invalid") from exc
                self._json(HTTPStatus.CREATED, self.runtime.service.import_design_bundle(owner_id, match["project_id"], content))
                return
            match = self._route(rf"/api/v1/projects/{PROJECT}/model-policy", path)
            if match:
                if not self._require_idempotency_key():
            ...
            if match:
                ...
                self._json(HTTPStatus.OK, result)
                return
            match = self._route(rf"/api/v1/projects/{PROJECT}/plans/(?P<plan_id>plan_[a-f0-9]{{32}})/restart", path)
            if match:
                if not self._require_idempotency_key():
                    return
                self._json(HTTPStatus.CREATED, self.runtime.service.restart_generation_plan(owner_id, match["project_id"], match["plan_id"]))
                return
            ...
            if match:
                ...
                return
            match = self._route(rf"/api/v1/projects/{PROJECT}/plans/(?P<plan_id>plan_[a-f0-9]{{32}})/content-export", path)
            if match:
                if not self._require_idempotency_key():
                    return
                self._json(HTTPStatus.CREATED, self.runtime.service.export_content_plan(owner_id, match["project_id"], match["plan_id"], str(body.get("format") or "markdown")))
                return
            match = self._route(rf"/api/v1/projects/{PROJECT}/plans/(?P<plan_id>plan_[a-f0-9]{{32}})/content", path)
            if match:
                if not self._require_idempotency_key():
                    return
                result = self.runtime.service.update_generation_plan_content(
                    owner_id,
                    match["project_id"],
                    match["plan_id"],
                    list(body.get("page_drafts") or []),
                    storyline=list(body["storyline"]) if isinstance(body.get("storyline"), list) else None,
                    confirm_invalidation=bool(body.get("confirm_invalidation", False)),
                )
                self._json(HTTPStatus.OK, result)
                return
            if path == "/api/v1/admin/prompt-retention/cleanup":
                if not self._require_admin(user) or not self._require_idempotency_key():
                    return
                self._json(HTTPStatus.OK, self.runtime.service.cleanup_expired_prompt_content(owner_id, limit=min(5000, max(1, int(body.get("limit") or 500)))))
                return
            match = self._route(rf"/api/v1/projects/{PROJECT}/operations", path)
            if match:
    ...
    def do_DELETE(self) -> None:
        ...
        if not user:
            return
        path = urlsplit(self.path).path
        design_match = self._route(rf"/api/v1/projects/{PROJECT}/design-packs/(?P<pack_id>[^/]+)", path)
        if design_match:
            if not self._require_idempotency_key():
                return
            try:
                self._json(HTTPStatus.OK, self.runtime.service.archive_design_pack(user["user_id"], design_match["project_id"], design_match["pack_id"]))
            except NotFoundError as exc:
                self._error(HTTPStatus.NOT_FOUND, "not_found", str(exc))
            return
        match = self._route(r"/api/v1/admin/provider-profiles/(?P<profile_id>[^/]+)", path)
