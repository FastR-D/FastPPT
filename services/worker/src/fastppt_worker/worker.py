...
class Worker:
    ...
    def run_once(self) -> bool:
        self.heartbeat()
        now = time.monotonic()
        if now - self.last_retention_cleanup >= 3600:
            try:
                self.runtime.service.cleanup_expired_prompt_content("system:retention-worker")
                self.last_retention_cleanup = now
            except Exception:
                LOGGER.exception("prompt retention cleanup failed")
