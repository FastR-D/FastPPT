...
class ResilienceTests(TestCase):
    ...
    def test_replayed_parse_plan_and_operation_do_not_duplicate_results(self) -> None:
        with TemporaryDirectory() as temp_name:
            ...
            first_samples = service.confirm_generation_plan(owner_id, project_id, plan["plan_id"])
            second_samples = service.confirm_generation_plan(owner_id, project_id, plan["plan_id"])
            self.assertEqual(first_samples["sample_pages"], second_samples["sample_pages"])
            self.assertEqual(len(store.list_pages(project_id)), 1)
            first = service.confirm_generation_samples(owner_id, project_id, plan["plan_id"])
            second = service.confirm_generation_samples(owner_id, project_id, plan["plan_id"])
