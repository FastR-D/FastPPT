...
class ReleaseGateTests(TestCase):
    ...
    def test_unregistered_image_change_fails_before_output_persistence(self) -> None:
        with TemporaryDirectory() as temp_name:
            service, store, owner_id, project_id = self._service(Path(temp_name))
            page = service.materialize_pages(
                owner_id,
                project_id,
                [{"title": "Image target", "body": "Editable", "page_type": "content"}],
            )[0]

            async def planner_with_unknown_image(settings, request, **kwargs):
                return AgentResult(
                    output={
                        "workflowMode": request.metadata["workflow_mode"],
                        "targetScope": request.metadata["target_scope"],
                        "affectedPageIds": list(request.metadata["page_ids"]),
                        "changes": [{
                            "kind": "image_change",
                            "target": "image",
                            "value": {"artifactId": "artifact_" + "f" * 32},
                            "factId": None,
                            "constraint": None,
                        }],
                        "pageDelta": {"add": [], "remove": [], "split": [], "merge": []},
            ...
            with patch.object(service.harness, "run", new=planner_with_unknown_image):
                with self.assertRaises(ConflictError):
                    service.create_edit_operation(owner_id, project_id, "Use this image", "single", [page["page_id"]])

            run = next(item for item in store.list_agent_runs(project_id) if item["role"] == "edit_planner")
            self.assertEqual(run["status"], "failed")
            self.assertEqual(run["error"]["code"], "unregistered_image_artifact")
            self.assertEqual(run["output_artifact_ids"], [])
            usage = store.usage_by_request(run["usage_request_id"])
            self.assertIsNotNone(usage)
            self.assertEqual(usage["submission_status"], "failed")
            self.assertEqual(store.list_operations(project_id), [])
            self.assertFalse(any(
                item["action"] == "agent.output.applied"
                and item.get("detail", {}).get("agent_run_id") == run["agent_run_id"]
                for item in store.list_project_audit(project_id)
            ))
            self.assertTrue(any(
                item["event_type"] == "agent.run.failed"
                and item["payload"].get("agent_run_id") == run["agent_run_id"]
                for item in store.events_after(project_id, 0)
            ))
