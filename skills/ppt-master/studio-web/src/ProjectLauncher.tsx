...
export function ProjectLauncher({ current, onEnter }: { current: ProjectInfo; onEnter: () => void }) {
  ...
  const refresh = () => void request<{ projects: RecentProject[] }>("/api/projects").then((result) => setProjects(result.projects));
  ...
  const edit = async (project: RecentProject) => { const nextName = window.prompt("项目名称", project.name); if (!nextName) return; const nextNote = window.prompt("项目备注", project.note ?? ""); try { await request(`/api/projects/${encodeURIComponent(project.projectId)}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: nextName, note: nextNote }) }); refresh(); } catch (error) { setNotice((error as Error).message); } };
  const remove = async (project: RecentProject) => { if (!window.confirm(`确认删除项目“${project.name}”？`)) return; try { await request(`/api/projects/${encodeURIComponent(project.projectId)}`, { method: "DELETE" }); refresh(); } catch (error) { setNotice((error as Error).message); } };

  const validName = (value: string) => value.trim().length >= 1 && value.trim().length <= 80;
  const open = async (value: string, action: "open" | "create") => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        try { const health = await fetch(new URL("/healthz", result.url), { cache: "no-store" }); if (health.ok) break; } catch { /* service is still starting */ }
  ...
  return <div className="launcher">
    <section className="launcher-panel">
      ...
      <section className="recent-projects"><div><div><b>最近项目</b><small>{projects.length} 个可用工作区</small></div><button className="primary" onClick={() => { setName(""); setNote(""); setNotice(""); setCreateOpen(true); }}>＋ 新建演示文稿</button></div><div className="project-cards">{projects.map((project) => <article className="project-card" key={project.projectId}><button disabled={busy} onClick={() => void open(project.projectId, "open")}><span className="project-icon">F</span><strong>{project.name}</strong><code>ID · {project.projectId}</code>{project.note && <p>{project.note}</p>}<small>最近更新 · {new Date(project.updatedAt).toLocaleString("zh-CN")}</small><small>创建时间 · {new Date(project.createdAt).toLocaleString("zh-CN")}</small></button><div className="project-actions"><button onClick={() => void edit(project)}>编辑</button><button onClick={() => void remove(project)}>删除</button></div></article>)}</div>{!projects.length && <p className="launcher-empty">尚未发现 FastPPT 项目。</p>}</section>
    ...
    {createOpen && <div className="launcher-dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setCreateOpen(false); }}>
      <form className="launcher-dialog" role="dialog" aria-modal="true" aria-labelledby="create-project-title" onSubmit={(event) => { event.preventDefault(); if (validName(name) && !busy) void open(name, "create"); }}>
        ...
        <footer><button type="button" disabled={busy} onClick={() => setCreateOpen(false)}>取消</button><button className="primary" type="submit" disabled={busy || !validName(name)}>{busy ? "正在创建…" : "创建并打开"}</button></footer>
      </form>
    </div>}
  </div>;
}
