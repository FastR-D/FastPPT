...
import { commitGeneratedSlides, commitStaging, prepareStaging, recoverTransactions } from "./transaction.js";
...
const svgObjects = (content: string) => {
  ...
  const visit = (nodes: unknown) => { if (!Array.isArray(nodes)) return; for (const node of nodes as Record<string, unknown>[]) { const attrs = node[":@"] as Record<string, unknown> | undefined; if (attrs?.["@_id"]) objects.set(String(attrs["@_id"]), { fingerprint: hashText(JSON.stringify(node)), bbox: ["x", "y", "width", "height"].map((key) => Number(attrs[`@_${key}`] ?? 0)) }); for (const [key, value] of Object.entries(node)) if (key !== ":@") visit(value); } };
...
const server = Fastify({ logger: loggerOptions, disableRequestLogging: true, forceCloseConnections: true });
...
const defaultExporter = async () => {
  ...
  else if (quickGenerate) args.push("--quick-generate");
...
server.post("/api/workflow/research/run", async (_request, reply) => {
  ...
  try {
    ...
    if (overrides.researcher) text = await overrides.researcher(intake.provider, prompt, root);
      ...
      for await (const message of query({ prompt, options: { cwd: root, permissionMode: "acceptEdits", ...(nativeSessionId ? { resume: nativeSessionId } : {}) } })) {
...
server.post("/api/workflow/plan/run", async (_request, reply) => {
  ...
  try {
    ...
    else if (intake.provider === "codex") { const codex = new Codex(); const thread = typeof session.nativeSessionId === "string" ? codex.resumeThread(session.nativeSessionId) : codex.startThread({ workingDirectory: root, skipGitRepoCheck: true }); const turn = await thread.run(prompt); text = turn.finalResponse; if (thread.id) await atomicJson(sessionFile, { ...session, nativeSessionId: thread.id, status: "idle", updatedAt: now() }); }
    else { text = ""; let nativeSessionId = typeof session.nativeSessionId === "string" ? session.nativeSessionId : undefined; for await (const message of query({ prompt, options: { cwd: root, permissionMode: "plan", ...(nativeSessionId ? { resume: nativeSessionId } : {}) } })) { if (typeof message.session_id === "string") nativeSessionId = message.session_id; if (message.type === "result" && message.subtype === "success") text = message.result; } await atomicJson(sessionFile, { ...session, nativeSessionId, status: "idle", updatedAt: now() }); }
...
server.post("/api/workflow/generate-first", async (_request, reply) => {
  ...
  const prompt = `Author exactly one editable SVG page at svg_output/${first.id}.svg for this confirmed plan. Use viewBox ${canvas}. Return the complete SVG text only. Do not write outside staging.\n\n${JSON.stringify({ page: first, canvas: plan.canvas }, null, 2)}`;
  ...
  try {
    ...
    else if (provider === "codex") { const turn = await new Codex().startThread({ workingDirectory: staging, skipGitRepoCheck: true }).run(prompt); content = turn.finalResponse; }
    else { content = ""; for await (const message of query({ prompt, options: { cwd: staging, permissionMode: "acceptEdits" } })) if (message.type === "result" && message.subtype === "success") content = message.result; }
    ...
    const stagingFile = path.join(staging, "svg_output", `${first.id}.svg`); await writeFile(stagingFile, svgMatch[0], "utf8");
    ...
    const targetDirectory = path.join(root, "svg_output"); await mkdir(targetDirectory, { recursive: true }); const target = path.join(targetDirectory, `${first.id}.svg`), temporary = `${target}.studio-tmp`; await copyFile(stagingFile, temporary); await rename(temporary, target);
...
server.post("/api/workflow/generate-all", async (_request, reply) => {
  ...
  try {
    for (const page of plan.pages) {
      const prompt = `Author exactly one editable SVG page at svg_output/${page.id}.svg for this confirmed page. Use viewBox ${canvas}. Return the complete SVG text only. Maintain visual continuity across the deck. Do not write outside staging.\n\n${JSON.stringify({ page, pages: plan.pages, canvas: plan.canvas }, null, 2)}`;
      let content: string; if (overrides.generator) content = await overrides.generator(provider, prompt, staging); else if (provider === "codex") content = (await new Codex().startThread({ workingDirectory: staging, skipGitRepoCheck: true }).run(prompt)).finalResponse; else { content = ""; for await (const message of query({ prompt, options: { cwd: staging, permissionMode: "acceptEdits" } })) if (message.type === "result" && message.subtype === "success") content = message.result; }
      const svgMatch = content.match(/<svg[\s\S]*<\/svg>/i); if (!svgMatch || /<script\b|\son\w+\s*=|javascript:/i.test(svgMatch[0])) throw new Error(`generator did not return a safe SVG for ${page.id}`); await writeFile(path.join(stagingAuthoring, `${page.id}.svg`), svgMatch[0], "utf8"); await events.emit("generation.progress", { jobId, sessionId, slide: page.id });
    }
    ...
    const targetDirectory = path.join(root, "svg_output"); await commitGeneratedSlides(root, interaction, jobId, plan.pages.map((page) => page.id)); await finalizeGeneratedContent(jobId, plan.pages);
...
server.get("/api/accessories/notes", async () => { const roster = (await slideFiles(root)).map((name) => path.basename(name, ".svg")); const notes: Record<string, string> = {}; for (const slide of roster) { const file = path.join(root, "notes", `${slide}.md`); notes[slide] = await exists(file) ? await readFile(file, "utf8") : ""; } return { roster, notes, complete: roster.length > 0 && roster.every((slide) => notes[slide].trim()) }; });
server.post<{ Body: { slide?: string; intent?: string } }>("/api/accessories/notes/generate", async (request, reply) => { const roster = (await slideFiles(root)).map((name) => path.basename(name, ".svg")); if (!roster.length) return reply.code(409).send({ error: "暂无页面可生成讲稿" }); const targets = request.body.slide ? roster.filter((slide) => slide === request.body.slide) : roster; if (request.body.slide && !targets.length) return reply.code(404).send({ error: "页面不存在" }); const plan = await readJson<{ pages?: { id: string; role?: string; title?: string }[] }>(path.join(interaction, "page_plan.json"), {}); const pages = (plan.pages ?? []).filter((page) => targets.includes(page.id)); await finalizeGeneratedContent(`notes-${Date.now()}`, pages.length ? pages : targets.map((id) => ({ id }))); return { status: "completed", slides: targets }; });
...
server.get("/api/slides", async () => {
  ...
  const active = new Map<string, string>(); for (const jobId of await readdir(path.join(interaction, "jobs"))) { const job = await readJson<{ status?: string; targets?: { slide: string }[]; pages?: { id: string }[] }>(path.join(interaction, "jobs", jobId, "request.json"), {}); for (const slide of [...(job.targets ?? []).map((target) => target.slide), ...(job.pages ?? []).map((page) => page.id)]) if (["executing", "validating", "committing", "exporting", "summarizing"].includes(job.status ?? "")) active.set(slide, job.status === "validating" ? "generating" : "modifying"); else if (job.status === "failed") active.set(slide, "validation_failed"); }
  const roster = [...new Set([...(plan.pages ?? []).map((page) => page.id), ...existing.keys()])];
  return { slides: roster.map((id) => { let status = active.get(id); const revision = existing.get(id); if (!status) status = !revision ? plan.status === "awaiting_confirmation" ? "awaiting_confirmation" : plan.status ? "planning" : "ungenerated" : state.exportRevision && state.exportRevision === currentDeck ? "exported" : "exportable"; return { id, revision: revision ?? null, status, statusLabel: labels[status] }; }) };
});
...
server.get<{ Params: { jobId: string; id: string } }>("/api/jobs/:jobId/staging/:id/raw", async (request, reply) => { if (!safeId.safeParse(request.params.jobId).success || !safeId.safeParse(request.params.id).success) return reply.code(400).send({ error: "invalid staging reference" }); const file = path.resolve(interaction, "jobs", request.params.jobId, "staging", "svg_output", `${request.params.id}.svg`); const stagingRoot = path.resolve(interaction, "jobs", request.params.jobId, "staging"); if (!file.startsWith(`${stagingRoot}${path.sep}`) || !(await exists(file))) return reply.code(404).send({ error: "staging slide not found" }); const svg = await readFile(file, "utf8"); if (/<script\b|\son\w+\s*=|javascript:/i.test(svg)) return reply.code(422).send({ error: "unsafe staging SVG" }); return reply.type("image/svg+xml").send(svg.replace(/<svg\b([^>]*)>/i, (_match, attrs) => `<svg${attrs.replace(/\swidth="[^"]*"/i, " width=\"100%\"").replace(/\sheight="[^"]*"/i, " height=\"100%\"")} preserveAspectRatio="xMidYMid meet">`)); });
...
server.get<{ Params: { id: string } }>("/api/jobs/:id/diff", async (request, reply) => {
  ...
  for (const slide of [...new Set((job.targets ?? []).map((target) => target.slide))]) {
    const beforeFile = path.join(projectAuthoring, `${slide}.svg`), afterFile = path.join(stagingAuthoring, `${slide}.svg`);
...
server.post<{ Params: { id: string }; Body: { provider: "claude" | "codex" } }>("/api/jobs/:id/run-agent", async (request, reply) => {
  ...
  const prompt = `${await readFile(jobFile, "utf8")}\n\nAccepted project memories:\n${memoryContext || "(none)"}\n\nOnly edit SVG files under ${stagingAuthoring}. Do not edit the source project. Before finishing, run this exact validation command and repair the authored SVG until it exits successfully:\n${checkerCommand}`;
  ...
  if (overrides.agent) {
    ...
    await events.emit("validation.passed", { jobId: request.params.id }); await atomicJson(jobFile, { ...job, status: "committing", provider: request.body.provider }); await commitStaging(root, interaction, request.params.id, slides); await events.emit("revision.committed", { jobId: request.params.id, slides });
  ...
  if (request.body.provider === "codex") {
    ...
    await commitStaging(root, interaction, request.params.id, slides);
  ...
  let text = "", nativeSessionId = typeof session.nativeSessionId === "string" ? session.nativeSessionId : undefined; for await (const message of query({ prompt, options: { cwd: staging, permissionMode: "default", abortController, ...(nativeSessionId ? { resume: nativeSessionId } : {}) } })) { if (typeof message.session_id === "string") nativeSessionId = message.session_id; if (message.type === "result" && message.subtype === "success") text = message.result; }
...
server.post("/api/validate", async (_request, reply) => { const jobs = await readdir(path.join(interaction, "jobs")); let failed = false; for (const job of jobs) { const item = await readJson<{ type?: string; status?: string }>(path.join(interaction, "jobs", job, "request.json"), {}); if (item.type === "generation" && item.status === "failed") failed = true; } if (!(await exists(path.join(root, "svg_output"))) || !(await slideFiles(root)).length) { if (failed) return reply.code(422).send({ status: "failed", returncode: 1, stdout: "", stderr: "最近生成 staging 检查失败，请修复后重新检查" }); return { status: "not_started", returncode: 0, stdout: "No pages generated yet", stderr: "" }; } const result = await runChecker(root); await events.emit(result.code === 0 ? "validation.passed" : "validation.failed", { returncode: result.code }); return reply.code(result.code === 0 ? 200 : 422).send({ status: result.code === 0 ? "passed" : "failed", returncode: result.code, stdout: result.stdout, stderr: result.stderr }); });
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (existing.pid) { try { process.kill(existing.pid, 0); throw new Error(`FastPPT 已在端口 ${existing.port ?? "unknown"} 运行`); } catch (error) { if ((error as NodeJS.ErrnoException).code !== "ESRCH") throw error; } }
