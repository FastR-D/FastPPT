...
const planningSchema = z.object({ title: z.string().min(1), audience: z.string().min(1), tone: z.string().min(1), canvas: z.string().min(1), themeColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(), template: z.string().max(200).optional(), pages: z.array(z.object({ id: safeId, role: z.string().min(1), title: z.string().min(1) })).min(1) });
...
const waitForProjectStudio = async (baseUrl: string, project: string) => {
  ...
  for (let attempt = 0; attempt < 75; attempt += 1) {
    try {
      ...
      if (response.ok && health.status === "ok" && health.service === "project-studio-ts" && health.project === path.basename(project)) return;
...
server.post("/api/validate", async (_request, reply) => { const result = await runChecker(root); await events.emit(result.code === 0 ? "validation.passed" : "validation.failed", { returncode: result.code }); return reply.code(result.code === 0 ? 200 : 422).send({ status: result.code === 0 ? "passed" : "failed", returncode: result.code, stdout: result.stdout, stderr: result.stderr }); });
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (!process.argv[2]) {
    ...
    const candidates = await Promise.all((await readdir(projectsRoot, { withFileTypes: true })).filter((entry) => entry.isDirectory()).map(async (entry) => ({ name: entry.name, modified: (await stat(path.join(projectsRoot, entry.name))).mtimeMs })));
