...
class OpenAIImageAdapter:
    ...
    def _payload(settings: ImageSettings, request: ImageRequest) -> dict[str, object]:
        ...
        if request.input_images:
            media_types = request.input_media_types or ("image/png",) * len(request.input_images)
            if len(media_types) != len(request.input_images):
                raise ImageAdapterError("Image input media types do not match input images", code="validation_failed")
            payload["images"] = [
                f"data:{media_type};base64," + base64.b64encode(item).decode("ascii")
                for item, media_type in zip(request.input_images, media_types, strict=True)
            ]
        return payload
    ...
    def build_http_request(self, settings: ImageSettings, request: ImageRequest) -> Request:
        ...
        # Some HTTPS relays reject urllib's default Python user agent at their
        # edge. Send an explicit product identifier for provider diagnostics
        # and rate-limit handling without pretending to be a browser.
        headers = {
            "Content-Type": content_type,
            "Accept": "application/json",
            "User-Agent": "FastPPT/1.2.0",
        }
        if settings.api_key:
