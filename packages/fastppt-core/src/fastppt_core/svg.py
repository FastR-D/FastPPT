...
def _lines(text: str, width: int, limit: int) -> list[str]:
    ...
    for raw in text.splitlines():
        ...
        if not clean:
            continue
        # Hard-wrap unspaced CJK and long identifiers so SVG QA can reason
        # about each editable text line instead of rejecting an overflowing
        # single <text> node.
        result.extend(wrap(clean, width=width, break_long_words=True, break_on_hyphens=False) or [clean])
        if len(result) >= limit:
            break
    return result[:limit]
...
def render_page_svg(
    ...
    *,
    ...
    title_lines = _lines(title, 24 if title_size >= 38 else 30, 8) or ["Untitled"]
    title_line_height = 52 if title_size >= 48 else 44
    title_markup = "".join(
        f'<tspan x="84" dy="{0 if index == 0 else title_line_height}">{html.escape(line)}</tspan>'
        for index, line in enumerate(title_lines)
    )
    underline_y = 210 + (len(title_lines) - 1) * title_line_height
    content_offset = (len(title_lines) - 1) * title_line_height
    ...
    if layout == "two_column":
        ...
        for index, line in enumerate(stages):
            x = 84 + index * 370
            body_nodes.append(f'<rect x="{x}" y="{270 + content_offset}" width="320" height="170" rx="8" fill="#FFFFFF" stroke="#D8DCE1" stroke-width="2"/><text x="{x + 24}" y="{330 + content_offset}" font-family="Microsoft YaHei, Arial, sans-serif" font-size="24" fill="#25292D">{html.escape(line)}</text>')
            if index < 2:
                body_nodes.append(f'<line x1="{x + 325}" y1="{355 + content_offset}" x2="{x + 360}" y2="{355 + content_offset}" stroke="{accent}" stroke-width="5"/>')
    elif layout == "timeline":
        body_nodes.append(f'<line x1="120" y1="{355 + content_offset}" x2="1160" y2="{355 + content_offset}" stroke="{accent}" stroke-width="5"/>')
        for index, line in enumerate((body_lines + ["Milestone"] * 4)[:4]):
            x = 140 + index * 320
            body_nodes.append(f'<circle cx="{x}" cy="{355 + content_offset}" r="14" fill="{accent}"/><text x="{x - 50}" y="{410 + content_offset}" font-family="Microsoft YaHei, Arial, sans-serif" font-size="21" fill="#25292D">{html.escape(line)}</text>')
    elif layout == "metric_grid":
        for index, line in enumerate((body_lines + ["Metric"] * 4)[:4]):
            x, y = 84 + (index % 2) * 555, 245 + content_offset + (index // 2) * 165
    ...
    return "\n".join(
        (
            ...
            f'<text x="84" y="178" font-family="Microsoft YaHei, Arial, sans-serif" font-size="{title_size}" font-weight="700" fill="#171A1D">{title_markup}</text>',
            f'<rect x="84" y="{underline_y}" width="96" height="6" fill="{accent}"/>',
            *body_nodes,
            '</g>',
