...
class ContractValidationError(ValueError):
    """Raised when an immutable artifact is not safe to persist."""
...
def validate_page_contract_v2(payload: Mapping[str, Any], *, verify_hash: bool = True) -> dict[str, Any]:
    """Validate and return a v2 PageContract without dropping extension fields."""
    return PageContractV2.from_dict(payload, verify_hash=verify_hash).to_dict()


def validate_style_package_manifest_v2(payload: Mapping[str, Any], *, verify_hash: bool = True) -> dict[str, Any]:
    return StylePackageManifest.from_dict(payload, verify_hash=verify_hash).to_dict()


def validate_template_package_manifest_v2(payload: Mapping[str, Any], *, verify_hash: bool = True) -> dict[str, Any]:
    return TemplatePackageManifest.from_dict(payload, verify_hash=verify_hash).to_dict()


# Short aliases make the v2 contract entry points discoverable to callers that
# already import validators from this module.
validate_page_contract_v2_document = validate_page_contract_v2
validate_style_manifest_v2 = validate_style_package_manifest_v2
validate_template_manifest_v2 = validate_template_package_manifest_v2
...
def validate_page_contract(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("schema_version") in {"1.1.0", "1.2.0"}:
    ...
    if payload.get("schema_version") not in {None, "1.0.0", "1.1.0", "1.2.0"}:
    ...
    if payload.get("schema_version") == "1.2.0":
        design = payload.get("design_snapshot")
        if not isinstance(design, Mapping):
            errors.append("design_snapshot must be an object")
        else:
            if design.get("selection_source", "none") not in {"none", "private", "system"}:
                errors.append("design_snapshot.selection_source is invalid")
            if not isinstance(design.get("capability_matrix", {}), Mapping):
                errors.append("design_snapshot.capability_matrix must be an object")
    if errors:
...
AGENT_RUN_TRANSITIONS: dict[str, set[str]] = {
    AgentRunStatus.NOT_REQUIRED.value: {AgentRunStatus.NOT_REQUIRED.value},
...
def validate_reconstruction_manifest(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if payload.get("schema_version") not in {"1.1.0", "1.2.0"}:
        errors.append("schema_version must be 1.1.0 or 1.2.0")
