...
illustration/lettering assets, icons, qualitative Structure, native shapes,
charts/tables, and formulas—and may combine any suitable subset on a page. It
then authors under the shared SVG standards, runs any selected
...
For Create Layout / Create Deck, `standard` and `fidelity` inspect the complete source Master/Layout inventory as structural and visual evidence, then author complete Slide SVG prototypes and a new Master/Layout/slot system. `standard` stays compact; `fidelity` covers a broader set of useful source-aligned families. Every retained authored Layout is represented by at least one Slide prototype. `mirror` emits only source Slide prototypes and preserves each Slide's reachable Layout/Master chain; each standalone SVG resolves Master + Layout + Slide context while retaining explicit ownership markers. Source structures unused by every Slide remain evidence only. Layout mirror is legal only when the retained source is already brand-neutral and application-neutral; otherwise author a new Layout or retain the facts as Deck. Fixed-layer source groups are mechanically expanded into direct atoms while preserving ownership, paint order, and appearance. Create Brand analyzes identity fragments only; Create Style extracts portable method/direction only. Neither enters structural replication strategies or produces an SVG roster.

Slide-only is an SVG-roster boundary, not a ban on deterministic support data. A PPTX-backed mirror may carry per-Master source Theme and native-payload sidecars; tools validate and consume them without exposing them as prototypes or page-authoring context.
...
**Default preparation has two clocks.** Topic Research supplies facts and identifies adopted webpages before final confirmation: it starts immediately for topic-only input, or after supplied material is converted/read when planning-critical factual gaps remain. It supplements only those gaps. After project initialization, `project_manager.py import-sources` imports the research/provenance pair as ordinary files and never expands the v1 facts JSON's `source_url` values. When the current AI editor provides an isolated worker with web/fetch access and write access to the declared outputs, the main agent defines the gaps and the worker writes the existing research/provenance artifacts, returning only a receipt; otherwise research runs in the main context. Independent AI / web / slice image acquisition runs only after final confirmation and the completed `design_spec.md §VIII` / `spec_lock.md`, then reaches a terminal status before Executor starts. If normal image search is exhausted, one relevant adopted URL may be fetched as a Markdown + companion-image source package; the image owner reviews that package and promotes only accepted files into the runtime image pool. Strategist also resolves, syncs, and validates a curated project icon pool while authoring the final plan. Image_Generator, Image_Searcher, and icon-sync tooling are preparation mechanisms under Strategist ownership, not independent decision owners. Quick generation uses those preparation mechanisms as needed under the current agent's in-context decisions, without inserting a confirmation gate.
...
**The Design Spec owns the image-execution path.** Final confirmation from either the UI or chat is first persisted as `AI Image Acquisition Path` in `design_spec.md §I`; Image_Generator selects API, host-native, or manual from that value and may not reopen the decision during execution. `image_gen.py --manifest` belongs only to API Path A. For a manifest under `<project>/images/` when `<project>/design_spec.md` exists, the CLI guard reads that persisted value directly: `api` / `auto` permit Path A, while `host-native` / `manual` and a missing or invalid value block it. The guard does not reopen or consume either confirmation channel.

**One coherent sheet per compatible visual family.** AI sheets prepare transparent illustration or lettering elements rather than complete page layouts. A family may include repeated title/corner chrome, dominant anchors, supporting figures, and accents with different silhouettes and visual weights; compatible elements share a sheet and split only when cell geometry, detail, quality, or semantic needs conflict. SVG may combine the slices with backgrounds, native shapes, editable text, photos, other slices, and lettering on any suitable page, and one slice may recur across several pages. Decorative lettering remains grouped by visual identity, with controlled glyph-native expression by default and strong or lettering-plus-illustration expression only after an explicit request or confirmed direction. Every sheet uses an explicit flat pure-red/green/blue chroma key whose active channel does not dominate its elements; the same HEX is passed to `slice_images.py --bg`, whose chroma path removes key spill and recovers partial effect alpha. `--strict-alpha` then rejects any nontransparent cell boundary or edge-crossing effect before writing derived files. Successful transparent slices join `images/`, then `analyze_images.py` refreshes their dimensions.
...
**Why imported and authored shape metadata are separate.** A lossless imported SVG may need native-shape metadata, hidden carriers, and preview fingerprints to describe an advanced PowerPoint shape. That representation stays immutable as source/package evidence and supported non-visible payload backing. `svg_authoring_view.py` creates new compact editable SVG with document-local source refs plus an `authoring_manifest.json` containing paths and initial hashes. `standard` / `fidelity` author project-canonical SVG. Mirror also reviews/authors the compact parsed SVG, preserving structure, meaning, and similar presentation without code isomorphism. Its materializer validates/composes the current visible tree and never restores ordinary lossless subtrees; IR-only refs do not enter final templates.
...
named cell styles; native export expands those values in memory. Imported
objects declare `data-pptx-native-authority="json"`: inline JSON is
authoritative and the compact browser preview may be approximate. Meaning-bearing
...
**Why Create Layout / Create Deck have authored and preservation modes.** `pptx_template_import.py` emits new compact editable Master/Layout/Slide SVG plus native structure facts. `standard` / `fidelity` inspect the complete source evidence, then author compact or broader useful prototypes and topology. Mirror reviews/authors each compact parsed source Slide and only its reachable Layout/Master chain one-to-one. It preserves structure, meaning, and similar presentation without requiring SVG-code identity; the materializer validates/composes the current authored tree and never restores ordinary visible lossless subtrees. Unreferenced source structures are not mirror output. The original PPTX remains immutable validation evidence, not a packaged dependency. Create Brand has no structural replication strategy.
...
**Why PowerPoint-native Chart/Table reconstruction uses explicit replacement markers, not automatic object replacement.** The standalone `pptx_to_svg.py` importer emits a visible SVG preview plus `data-pptx-replace-with`, `data-pptx-native-authority="json"`, and inline `<metadata type="application/json">` only for validated table/chart subsets. Imported/template-owned objects are JSON-first; their preview is derived and may be approximate. Free-designed objects omit the authority marker, remain SVG-first, and stamp a fallback baseline only after visible SVG and JSON are synchronized. Generated decks prepare the pair only for semantic object keys recorded as `<object-key>=yes` in the §IX `Native-ready` map. The parent marker determines the payload schema; ordinary shapes and connectors never use this contract. Table markers require `ppt-master.semantic-table.v2`: import factors exact repeated cell/paragraph/run values into defaults and named cell styles, while export expands them before native validation. Table import covers exact physical row/grid topology, canonical rectangular merges with empty slaves, safe solid/no-fill per-side borders, plain multi-paragraph cells, and a closed run-rich paragraph form. A rich paragraph contains non-empty `runs`; every run requires `text` and may use only `bold`, `italic`, `underline`, `strike`, `color`, `font_size`, one `font_family`, `lang`, and `alt_lang`. Unknown source presentation-only run XML without a non-empty `effectLst` / `effectDag` normalizes into that schema; a table-cell run effect instead disables native replacement and adds a blocking effect diagnostic. Relationship-bearing text, extensions, line breaks, fields, tabs, bullets, malformed text topology, noncanonical merges, unsafe borders, and non-solid fills remain fallback-only.
